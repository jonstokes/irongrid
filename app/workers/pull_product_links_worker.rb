class PullProductLinksWorker < BaseWorker
  ...
  def call
    ring "Pulling product links for #{site.domain} with queue size #{site.product_links_queue.size}"
    ...
    ring "Pulled #{record[:data][:links_created]} product links for #{site.domain}. Queue size is #{site.product_links_queue.size}"
  end
