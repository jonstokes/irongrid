...
describe Stretched::ObjectQueue do
  ...
  before :each do
    ...
    @store = Stretched::ObjectQueue.find("www.retailer.com")
  ...
  describe "#pop", no_es: true do
    it "should return an object and remove it from the queue" do
      ...
      expect(@store.has_key?(ObjectQueue.key(object)).to be_false
      expect(ObjectQueue.get(ObjectQueue.key(object)).to be_nil
      expect(object).to be_a(ObjectQueue)
