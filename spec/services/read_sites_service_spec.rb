...
describe ReadSitesService do
  ...
  describe "RefreshLinksWorker sites" do
    ...
    it "should generate a RefreshLinksWorker for a site if it should be read and its LinkQueue is empty" do
    ...
    it "should not generate a RefreshLinksWorker for a site if it should not be read and its LinkQueue is empty" do
    ...
    it "should not generate a RefreshLinksWorker for a site if it should be read and its LinkQueue is not empty" do
      @site.update(read_at: Time.now)
      @lq = LinkQueue.new(domain: @site.domain)
      @lq.push "http://#{@site.domain}/1"
      @service.start
      @service.stop
      expect(RefreshLinksWorker.jobs_in_flight_with_domain(@site.domain)).to be_empty
    end
